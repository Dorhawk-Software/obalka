...
def render(size=1024):
    # A private directory rather than `tempfile.mktemp`, which only names a path: another process could
    # create that file between the naming and Chrome writing it (CodeQL py/insecure-temporary-file).
    # The directory also goes away afterwards; the old path was never deleted.
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # convert() reads the pixels now, before the directory and the file in it are removed.
