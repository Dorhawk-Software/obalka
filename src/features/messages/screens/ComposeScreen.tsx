...
export const ComposeScreen = forwardRef<ComposeHandle, ComposeScreenProps>(
  ...
  function ComposeScreenView({ account, onBack, onOpenSent }, ref) {
    ...
    const doSend = useCallback(
      async (confirmedPaid: boolean) => {
        ...
        switch (out.kind) {
          case 'sent':
            ...
            recorded.current = messagesController.recordSentMessage(account, {
      ...
      [account, recipient, subject, files, bodyText],
    );
          ...
          >
            ...
            <PressScale
              onPress={() => {
                ...
                setOpening(true);
                const id = sentId;
                void Promise.resolve(recorded.current).then(() => {
                  onOpenSent(id);
                });
              }}
