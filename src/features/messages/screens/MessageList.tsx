...
export function MessageList({
    ...
    async (ctrl: AbortController, isUserRefresh: boolean): Promise<void> => {
        ...
        ? await messagesController
            ...
            .then(merged => {
              ...
              return {
                ...
                syncedAt: merged.syncedAt ?? Date.now(),
              };
            })
      ...
      if (outcome.kind === 'loaded') {
        setState({
          ...
          // pass, but reading it back would cost a round trip to say what we already know.
          syncedAt: outcome.syncedAt ?? Date.now(),
        });
      } else {
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      draftsStore.list(account.boxId).then(drafts => {
        if (alive) {
