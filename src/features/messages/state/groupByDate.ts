...
export function freshnessLabel(syncedAt: number | null, now: number): string | null {
  if (syncedAt == null || !Number.isFinite(syncedAt) || syncedAt > now + 60_000) {
    return null; // never synced, or a clock that disagrees - say nothing rather than something wrong
  }
