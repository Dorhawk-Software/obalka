...
export class BackupController {
   ...
   */
  async enable(promptTitle: string, onProgress?: ProgressFn): Promise<BackupManifest> {
    if (!(await this.deps.secret.canProtect())) {
      throw new BackupLockUnavailableError();
    }
    if (await this.deps.secret.has()) {
      return this.backupNow(promptTitle, onProgress);
    }
  ...
  async backupNow(
    ...
    return this.track('backup', (progress, stop) =>
      this.backupWith(passphrase, stop, combine(onProgress, progress)),
    );
  }
  ...
  async preferences(): Promise<BackupPreferences> {
    const [auto, keep, documents] = await Promise.all([
  ...
  private async restoreFromStore(
    ...
    this.archiveChanged();
    return { ...report, keysFailed: !kept, held: holds && notBack > 0 };
  }
  ...
  private async backupWith(
    ...
    const documents = this.deps.documents;
    const tierOn = (await this.preferences()).documents;
