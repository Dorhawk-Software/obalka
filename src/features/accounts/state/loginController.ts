...
export class LoginController {
  ...
  /** Begin a login with the entered credentials (any method). */
  async start(input: Credentials): Promise<void> {
  ...
  /** Submit a one-time code after an OTP prompt. */
  async submitOtp(code: string): Promise<void> {
    this.inFlight = new AbortController();
    this.dispatch({ type: 'SUBMIT_OTP' });
    await this.applyOutcome(await this.deps.authService.submitOtp(code, this.inFlight.signal));
  }

  /** Re-request the TOTP SMS. */
  async resendSms(): Promise<void> {
    this.inFlight = new AbortController();
    this.dispatch({ type: 'RESEND_SMS' });
    await this.applyOutcome(await this.deps.authService.resendSms(this.inFlight.signal));
  }
  ...
  private async applyOutcome(outcome: LoginOutcome): Promise<void> {
    ...
    this.dispatch({ type: 'OUTCOME', outcome: await this.explainRefusal(outcome) });
  }
