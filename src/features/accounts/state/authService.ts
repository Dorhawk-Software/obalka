...
export class IsdsAuthService {
   ...
   * the latest (`endHandshake`). The code screen offers to send the code again while the first SMS
   * request is still out, and a cancel aborts only the request the controller holds, so that first
   * request runs on past a resend or a new sign-in - and failing there, it emptied the handshake the
   * resent code, or the new sign-in, had to ride.
   */
  private handshake = 0;
