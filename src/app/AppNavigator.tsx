...
function TransferRoute({
  ...
  const [newest, setNewest] = useState<BackupManifest | null>(null);
  useEffect(() => {
    // The newest backup is what a transfer sends, and it is read here rather than passed in so the
    // screen cannot be opened holding a stale one.
    void backupController
      .status()
      .then(s => setNewest(s.last))
