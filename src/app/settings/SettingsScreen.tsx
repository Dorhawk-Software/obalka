...
import { APP_VERSION, REPOSITORY_LABEL, REPOSITORY_URL } from '../appInfo';
import { useSettings, type ThemeMode } from './SettingsProvider';

const THEME_MODES: ThemeMode[] = ['light', 'dark', 'system'];

/**
 * Hands the repository URL to the OS browser. The ONLY outbound action in the About area, and it is a
 * hand-off rather than a fetch - nothing in this app talks to a server of ours (Principle III). A
 * failure here is inert: the row simply does nothing rather than surfacing an error the user cannot act
 * on.
 */
function openRepository(): void {
  void Linking.openURL(REPOSITORY_URL).catch(() => {});
}

// Drawn 28×20 flag chips (design) - NOT the regional-indicator emoji, which most Android builds render
// as bare "CZ" / "GB" letters.
const LANGUAGES: { code: Locale; flag: ReactNode }[] = [
  { code: 'cs', flag: <FlagCz /> },
  { code: 'en', flag: <FlagEn /> },
];
      ...
      >
        ...
        <Section label={t('settings.language')}>
          <OptionGroup
            ...
            onChange={setLocale}
            testIDPrefix="lang"
            options={LANGUAGES.map(({ code, flag }) => ({
              value: code,
        ...
        {/* One card, four rows, hairline-separated (design). The version row is a VALUE row; the rest
            navigate - the source-code row leaves the app, so it carries the external-link mark rather
            than a chevron. */}
        <Section label={t('settings.about')}>
          ...
          >
            ...
            <AboutRow last onPress={openRepository}>
