...
//
// CONSENT. Gated in the send hooks rather than at `init`, so flipping the switch takes effect on the
// next event instead of the next launch, and so there is exactly ONE place where the decision to
// transmit is made.
//
...
/**
 * Start the SDK. Safe to call when there is no DSN - it simply does nothing, which is the state of
 * a checkout that has not been pointed at a Sentry project.
 */
