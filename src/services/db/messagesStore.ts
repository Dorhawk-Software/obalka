    ...
    for (const e of envelopes) {
      ...
      await db.execute(
        `INSERT INTO messages
           (boxId, messageId, folder, subject, sender, senderAddress, recipient, recipientAddress, deliveryTime, acceptanceTime, state, attachmentSize, searchText, syncedAt)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
         ON CONFLICT(boxId, messageId) DO UPDATE SET
           ...
           searchText = excluded.searchText, syncedAt = excluded.syncedAt`,
        [
          boxId,
          e.id,
          folder,
