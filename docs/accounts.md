...
- A restore asked for while a removal is running shows no progress until the removal has ended: its
  ...
  (`BackupController.track`), after the wait.
- A removal whose row then stays (the row delete refused) has stopped the box's calls meanwhile, so an
  inbox open on that box can say its messages could not be loaded, until it is refreshed again.
...
sign-in has opened a handshake of its own. So removing a box finds nothing of it there to empty.
...
cancel has emptied the jar. Neither signs anyone in: finishing that handshake takes the password and a
new code, or the communication code. A jar that will not empty keeps what it held, a captured session
included, until a later sign-in empties it; the failure is reported (`isds.login`).
