...
4. **The pre-consent window.** Telemetry starts only once the persisted setting is read, so a crash
   ...
   under a default rather than the user's answer.
5. **JS source maps upload only when a token is present.** *Amended 2026-09-14* (this read "are not
   ...
   (`scripts/sentry-token.sh` installs it); there is no Android release workflow in CI. Without a
   token, a JS stack trace from a release bundle is still minified.
