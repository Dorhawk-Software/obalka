...
describe('offering a transfer', () => {
  ...
  it('sends the newest backup, when an automatic backup replaced the one the screen read (2026-09-15)', async () => {
    // The screen reads the newest backup when it opens. An automatic backup made meanwhile wrote a newer
    // one and, keeping one, deleted the archive the screen named: the send failed, and so did every
    // retry until the screen was opened again.
    const archive = await sealedArchive();
    const read = manifest({ createdAt: 1_000, archiveName: 'obalka-1000.backup' });
    const newer = manifest({
      createdAt: 2_000,
      archiveName: 'obalka-2000.backup',
      sizeBytes: archive.length,
    });
    const { controller, archives, store } = build(undefined, { manifests: [newer] });
    archives.set(newer.archiveName, archive);

    const offer = await controller.offer(read);
    const sent = unpackPortable(
      await store.fs.readBytes(`${runDirIn(store, 'out')}/${TRANSFER_BACKUP_FILE}`),
    );
    await offer.done;

    expect(sent.manifest.archiveName).toBe(newer.archiveName);
    expect(Buffer.from(sent.archive).equals(Buffer.from(archive))).toBe(true);
  });

  it('looks again when the archive goes between the listing and the read (2026-09-15)', async () => {
    // An old backup is pruned only once the newer one is written, so a second listing finds the newer.
    const archive = await sealedArchive();
    const read = manifest({ createdAt: 1_000, archiveName: 'obalka-1000.backup' });
    const newer = manifest({
      createdAt: 2_000,
    ...
    const { controller, archives, store } = build(undefined, {
      manifests: () => (++listings === 1 ? [read] : [newer]),
    });
    archives.set(newer.archiveName, archive);
...
describe('carrying the documents too (T014)', () => {
  ...
  it('states the size of what MOVES, not what the bundle weighs', async () => {
    ...
    const offer = await controller.offer(manifest({ sizeBytes: a.length }));
    await offer.done;
    const bundle_ = packPortable(manifest({ sizeBytes: a.length }), a);
  ...
  it('says it is PREPARING before a phrase exists, not that it is waiting', async () => {
    ...
    const stages: string[] = [];
    const offer = await controller.offer(manifest({ sizeBytes: a.length }), p =>
      stages.push(p.stage),
    );
    await offer.done;
...
describe('never holding a document whole (025 review, 2026-09-15)', () => {
  it('copies each document into the outgoing directory a slice at a time', async () => {
    ...
    expect(offer.sizeBytes).toBe(packPortable(manifest({ sizeBytes: a.length }), a).length + 1000);
  ...
  it('stops part-way through a large document when the transfer is stopped', async () => {
    ...
    await expect(
      controller.offer(manifest({ sizeBytes: a.length }), undefined, signal),
