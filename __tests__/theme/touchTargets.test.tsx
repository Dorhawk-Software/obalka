...
/** How tall a text button's line is drawn: its label's line box, or the icon beside it if taller. */
function lineOf(node: Host): number {
  ...
  return Math.max(
    ...parts.map(n => {
      ...
      return (n.type === 'Text' ? style.lineHeight : style.height) as number;
    }),
  );
}
...
describe('the help links', () => {
  ...
  it('on Welcome: a dense line with 6 above and below reaches the target', async () => {
    const view = await wrap(<Welcome onAddBox={() => {}} onOpenFaq={() => {}} />);
